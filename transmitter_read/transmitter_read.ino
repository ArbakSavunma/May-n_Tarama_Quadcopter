...
void receive() {
  ...
  receiver_values[rp] = map(pulseIn (receiver_pins[rp], HIGH), res_min, res_max, -1 * working_range, working_range); //kumandada bir channelda high olduğu süreyi hesaplayıp ona göre -255 ile 255 arasında değer atıyor
  rp++;
  if (rp == 6) {
    rp = 0;
  }
